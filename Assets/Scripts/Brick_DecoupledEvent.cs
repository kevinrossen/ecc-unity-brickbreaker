...
public class Brick_DecoupledEvent : MonoBehaviour
{
    ...
    public static event System.Action<Brick_DecoupledEvent, int> OnBrickHit;
    ...
    private void Hit()
    {
        ...
        // This decouples Brick from knowing about scoring, level progression, etc.
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.name == "Ball") {
            Debug.Log($"[{gameObject.name} #{GetInstanceID()}] Brick hit! Remaining health: {health}");
            Hit();
        }
    }
}